...
int main(int argc, char* argv[]) {
  ...
  // Bloque paralelo que instancia las primeras partículas del programa.
  ...
  while (!quit) {
    ...
    if (currentTime - lastFrameTime >= FPS_DELAY) {
      ...
      if (currentTime - lastNewParticleTime >= NEW_LINE_INTERVAL_MS) {
        ...
        // Bloque paralelo que instancia y coloca las nuevas partículas en el vector.
        #pragma omp parallel num_threads(threadCount)
        {
          int numThreads = omp_get_num_threads();
          int threadId = omp_get_thread_num();
      ...
      // Ciclo for paralelo que se encarga de actualizar todas las partículas.
